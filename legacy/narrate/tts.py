...
class Engine:
    ...
    def narrate(self, paragraphs: list[str], voice_id: str, speed: float,
                progress: Callable[[int, int, float], None], cancel: threading.Event
                ) -> tuple[np.ndarray, list[list[Word]]]:
        """Synthesize all paragraphs. Returns (audio, words-per-paragraph with absolute times).
        progress(done_chunks, total_chunks, seconds_so_far)."""
        plan = [(pi, c) for pi, p in enumerate(paragraphs) for c in chunk_paragraph(p)]
        total = len(plan)
        pause_chunk = np.zeros(int(SAMPLE_RATE * 0.18), np.float32)
        pause_para = np.zeros(int(SAMPLE_RATE * 0.55), np.float32)
        pieces: list[np.ndarray] = []
        words: list[list[Word]] = [[] for _ in paragraphs]
        offset = 0.0
        last_para = -1
        for i, (pi, text) in enumerate(plan):
            if cancel.is_set():
                raise InterruptedError
            if pieces:
                pause = pause_para if pi != last_para else pause_chunk
                pieces.append(pause); offset += len(pause) / SAMPLE_RATE
            audio, timed = self.synth_timed(text, voice_id, speed)
            pieces.append(audio)
            words[pi] += [Word(w.text, round(offset + w.start, 3), round(offset + w.end, 3)) for w in timed]
            offset += len(audio) / SAMPLE_RATE
            last_para = pi
            progress(i + 1, total, offset)
        if not pieces:
            return np.zeros(SAMPLE_RATE // 2, np.float32), words
        out = np.concatenate(pieces)
        peak = float(np.abs(out).max()) or 1.0
        return out * min(1.0, 0.95 / peak), words   # gentle peak normalisation, never clips
